...
public class KeycloakService {
    ...
    public String getClientSecret(String clientId) {
        ...
        List<ClientRepresentation> clients = keycloak
                .realm(keycloakEnvironment.getKEYCLOAK_REALM_BANK_NAME())
                .clients()
        ...
        return keycloak.realm(keycloakEnvironment.getKEYCLOAK_REALM_BANK_NAME()).clients().get(client.getId()).getSecret().getValue();
    }
    ...
    public void createUser(String email, String password, String name, String surname) throws Exception {
        ...
        try (Response createUserResponse = usersResource.create(user)) {
            if (createUserResponse.getStatus()/100 != 2) {
                throw new Exception(String.format("There is a problem while creating an user in realm %s with email %s.", keycloakEnvironment.getKEYCLOAK_REALM_BANK_NAME(), email));
            }
