...
public class KeycloakConfiguration {
    ...
    public KeycloakConfiguration(@Value("${keycloak.server.host}") String KEYCLOAK_SERVER_HOST,
        ...
        System.out.println(KEYCLOAK_REALM_BANK_NAME);
        ...
        this.keycloak = KeycloakBuilder.builder()
                ...
                .realm(KEYCLOAK_REALM_MASTER_NAME)
                .username(KEYCLOAK_ADMIN_NAME)
                .password(KEYCLOAK_ADMIN_PASSWORD)
                .clientId(KEYCLOAK_CLIENT_ADMIN_ID)
                .grantType(OAuth2Constants.PASSWORD)
                .build();
    }

    public UsersResource getUsersResource(){
        return keycloak.realm(KEYCLOAK_REALM_BANK_NAME).users();
    }

    public RoleResource getRoleResource(String role){
        return keycloak.realm(KEYCLOAK_REALM_BANK_NAME).roles().get(role);
    }
